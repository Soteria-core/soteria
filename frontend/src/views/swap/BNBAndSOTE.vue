    ...
    <el-form :model="form" :rules="rules" ref="form" :disabled="!member.isMember">
        ...
        <el-col :xs="24" :sm="11">
          <el-card class="box-card-from">
            <div slot="header" class="clearfix">
              ...
              <el-link
                v-else
                @click="form.amount=balanceValue.toString()"
                type="success"
                :underline="false"
            ...
            <div>
              <el-form-item prop="amount">
                <el-input
                  ...
                  v-model="form.amount"
                  @input="checkAmount">
                  <el-select v-model="form.type" slot="append" style="width: 85px;" @change="typeChange">
      ...
      <el-row>
        <div class="tip normal-text" v-if="member.isMember">
          ...
          <div>
            <svg-icon icon-class="circle" class="icon error-color"></svg-icon>
            When MCR% is less than 100%, SOTE cannot be exchanged back to BNB. But you can choose the wrapp token and
            sell it.
          </div>
        </div>
...
export default {
  ...
  watch: {
    ...
    'form.amount': function (newVal, oldVal) {
      this.$refs.form.validate((valid) => {
        const result = /^((0(\.[0-9]+)?)|([1-9][0-9]*(\.[0-9]+)?))$/g.test(newVal);
        if (result) {
          this.calc();
        }
      });
  ...
  methods: {
    ...
    checkAmount(value) {
      ...
      if (!+newValue) this.form.toAmount = '0';
      return newValue;
    },
    typeChange() {
      this.form.amount = '';
      this.form.toAmount = '0';
    }
  }
}
</script>
