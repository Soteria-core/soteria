    ...
    <el-card class="box-card">
      <el-form :model="options" :rules="rules" ref="form">
          ...
          <el-col :xs="24" :sm="12" style="margin-bottom: 10px">
            <el-card>
              <div slot="header" class="clearfix">
                ...
                <el-link
                  v-if="showReset"
                  @click="options.perAmount=''"
                  type="primary"
                  style="float: right; padding: 3px 0"
                  :underline="false">Reset
                </el-link>
                <el-link
                  v-else
                  @click="options.perAmount=balance.toString()"
                  type="success"
                  :underline="false"
                  style="float: right; padding: 3px 0">Max
                </el-link>
              </div>
              <div>
        ...
        <el-row class="normal-text">
          ...
          project.
          Stake your total deposit up to
          <highlight>10 times, {{ maxTotalAmountShow }} SOTE</highlight>
          .
      ...
      <el-table
          ...
          <template slot-scope="scope">
            <el-form :model="scope.row">
              <el-form-item :rules="scope.row.ownerStaked==0 ? rule : noRule" prop="stake">
                <el-input-number size="mini" v-model="scope.row.stake" class="right-input" :min="0">
                </el-input-number>
                SOTE
              </el-form-item>
            </el-form>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-link
              type="primary"
              v-if="scope.row.stakedStatus != 'staked'"
              :underline="false"
              @click="removeProject(scope)">Remove
            </el-link>
...
export default {
  ...
  computed: {
    ...
    showReset(){
      ...
      return BigNumber(depositAmount).comparedTo(0) > 0;
    },
    // 已经stake的总和
    usedAmount() {
      // 计算总和
    ...
    totalAmount(){
      const depositAmount = this.options.perAmount || 0
      return BigNumber(depositAmount).plus(this.options.totalAmount).toString();
    },
    // 总充值金额，perAmount本次充值金额，totalAmount已经充值金额
    totalAmountShow(){
      const depositAmount = this.options.perAmount || 0
      return BigNumber(depositAmount).plus(this.options.totalAmount||0).toFixed(2, 1).toString();
    },
    maxTotalAmountShow() {
  ...
  methods: {
    ...
    maxTotalAmount(){
      const depositAmount = this.options.perAmount || 0;
      const totalAmount = this.options.totalAmount || 0;
      this.options.maxTotalAmount = BigNumber(depositAmount.toString())
        .plus(totalAmount.toString())
